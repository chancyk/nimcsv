...
task sampleprof, "compile and run the sample":
    exec "nim cpp -r -d:release --stackTrace:on --debugger:native --gc:arc -o:./bin/nimcsv.exe src/main.nim"

task debug, "compile with debug flags":
    exec "nim cpp -r --stackTrace:on --debugger:native --gc:arc -o:./bin/nimcsv.exe src/main.nim"

task pymod, "build the nimpy python module":
    exec "nim cpp -d:release --app:lib --out:./bin/pynimcsv.pyd --threads:on --tlsEmulation:off --passL:-static --gc:arc ./src/pynimcsv.nim"
